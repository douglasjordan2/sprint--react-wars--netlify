...
React is a UI library whose purpose is to build powerful applications. React uses components that are self-contained and deal with their individual pieces of the DOM. React's virtual DOM updates the rendering of the applications. React is very efficient, and solves the problem of creating very large web applications that millions of users interact with simultaneously, offering them a smooth UX and assisting developers in creating the apps.

- [ ] What does it mean to _think_ in react?

Thinking in React involves the ability to translate web apps into individual components, and knowing where to store specific data about these components and the ability to pass that data in a useful way.

- [ ] Describe state.

State is the data stored in a parent component that provides context for its child components. When state is changed, React knows to rerender the app and updated data will be passed meaningfully to the child components.

- [ ] Describe props.

Props are data passed to different components. Props can be passed up or down, but they are not mutable. Props are considered read-only. Passing props up is possible, but generally frowned upon. Props provide context for child components. Components should never modify their own props.
